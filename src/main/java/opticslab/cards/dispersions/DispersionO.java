...
public class DispersionO extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "DispersionO", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        Color orangeC = Color.ORANGE.cpy();
        orangeC.a = 0.5F;
        ColorSpotlightPlayerEffect orange = new ColorSpotlightPlayerEffect(orangeC);
        AbstractDungeon.effectsQueue.add(orange);
        addToBot(new DrawCardAction(magicNumber));
    }

    @Override
    public AbstractCard makeCopy() {
        return new DispersionO();
    }
}
