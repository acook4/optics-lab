...
public class DispersionB extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "DispersionB", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        Color blueC = Color.BLUE.cpy();
        blueC.a = 0.5F;
        ColorSpotlightPlayerEffect blue = new ColorSpotlightPlayerEffect(blueC);
        AbstractDungeon.effectsQueue.add(blue);
        addToBot(new HealAction(p, p, magicNumber));
    }

    @Override
    public AbstractCard makeCopy() {
        return new DispersionB();
    }
}
