...
public class DispersionG extends BaseCard {
    private final static CardInfo cardInfo = new CardInfo(
            "DispersionG", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        Color greenC = Color.GREEN.cpy();
        greenC.a = 0.5F;
        ColorSpotlightPlayerEffect green = new ColorSpotlightPlayerEffect(greenC);
        AbstractDungeon.effectsQueue.add(green);
        addToBot(new ExhaustAction(magicNumber,false,true,true));
    }

    @Override
    public AbstractCard makeCopy() {
        return new DispersionG();
    }
}
